...
$T = 249$) windows (Table~\ref{tab:descriptive_stats}): excess kurtosis with
Jarque-Bera rejection, persistent volatility clustering, and
near-zero linear autocorrelation in raw returns
\cite{famaEMH1970}. With these data targets in hand, we organized
the remainder around three sequential questions: whether the hybrid
hidden Markov framework, once calibrated to those targets,
reproduced them in-sample and out-of-sample against six baseline
generators; whether the variance-corrected single-index composer
preserved the validated marginals when combined with a market factor
across a $424$-ticker universe against five alternative composers;
and whether the resulting paths supported a downstream
Value-at-Risk backtest at industry-standard coverage levels.
...
pipeline (Algorithm~\ref{alg:hybrid}) and asked whether those
validated marginals plugged into a multi-asset construction without
breaking either target. We evaluated six composers on a fixed set of
