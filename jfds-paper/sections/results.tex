...
We first tested whether the marginal generator in
Fig.~\ref{fig:model_architecture} reproduced the empirical SPY targets
shown in Fig.~\ref{fig:empirical_motivation}. In both the in-sample
...
Appendix~\ref{sec:supp-generator}). The primary analysis used $N = 100$
...
Among the fitted parametric and latent-state generators, the two HMM
variants gave the strongest overall distributional fit.
...
The jump episodes improved temporal fit. The HMM-WJ model reduced ACF-MAE from
$0.059$ for HMM-NJ to $0.053$. About $24\%$ of HMM-WJ paths contained
at least one jump and showed more persistent autocorrelation in
absolute returns; paths without a jump were equivalent to HMM-NJ.
...
(Supplementary Table~\ref{tab:supp-acf-prediction}). On
the held-out $249$ days of $2025$, HMM-NJ and HMM-WJ retained KS pass
rates of $96.6\%$ and $95.4\%$, with kurtosis close to the observed
...
to $0.0061$ (Supplementary Fig.~\ref{fig:cov_diag}). These values
supported the negligible-cross-term approximation used in
Eq.~\eqref{eq:scale}.
...
magnitude below the observed value (Supplementary Fig.~\ref{fig:tails}). Centered
...
(Supplementary Table~\ref{tab:allocator_decomp}). This comparison was
consistent with omitted residual covariance creating a
diversification-return bias, but it did not isolate how much of the
gap that omission caused. A constant log-wealth shift set the
synthetic ensemble median to the prespecified $8\%$ annual return
