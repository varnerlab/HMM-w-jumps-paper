...
(Table~\ref{tab:model_comparison}, Figure~\ref{fig:model_comparison}). Bootstrap resampling set the
...
kurtosis gap and only $68.7\%$ quantile coverage; a head-to-head
isolation of the persistence mechanism (HMM-WJ vs HSMM) and the
emission family (Gaussian vs Student-$t$) is given in
Table~\ref{tab:emission_hsmm} of
Sec.~\ref{sec:supp-emission-hsmm}. No single model
...
For each ticker and each replication, a single innovation path
drawn from the ticker's JumpHMM marginal was passed unchanged to
the naive, Gaussian-residual-SIM, and hybrid composers (a paired
innovation protocol), so that differences among those three
isolated the composition rule from generator stochasticity. The
\emph{Naive} composer plugged
the JumpHMM draw into the SIM identity with no variance correction.
The \emph{Gaussian SIM} baseline replaced the JumpHMM draw with
i.i.d.\ Gaussian noise scaled to the calibrated residual variance,
recovering $R^2$ exactly but discarding any marginal structure. The
\emph{Hybrid} composer applied Algorithm~\ref{alg:hybrid} with floor
$f = 0.10$ and threshold $R^2_{\mathrm{preserve}} = 0.80$. Three
further composers drew their innovations from dedicated residual
generators fit to the real OLS residual series:
\emph{JumpHMM-on-residuals} fitted a fresh JumpHMM marginal to the
residuals via a pseudo-price inversion; \emph{Block
bootstrap}~\cite{politisRomano1994} drew Politis--Romano stationary
bootstrap replicates of the empirical residual series with mean block
length $\sqrt{T} \approx 50$; \emph{GARCH(1,1)-$t$}~\cite{bollerslev1986}
fitted a conditional-variance model per ticker, with $30$ of the
$423$ non-stationary fits excluded from that composer only. We
skipped the optional cross-sectional rank reorder in
Algorithm~\ref{alg:hybrid} so per-ticker metrics reflected the
marginal construction alone; cross-sectional dependence under copula
reorder is outside the scope of the per-ticker evaluation reported
here.
...
the calibrated $\beta$ (Fig.~\ref{fig:preservation}). The variance-ratio panel made the mechanism explicit: the naive
scatter traced the theoretical curve $1 + \rho_i$, overshooting the
...
the consequence: naive fell from a pass rate of $\sim 0.25$ at
$\beta \approx 0.3$ to below $0.01$ past $\beta \approx 0.7$, while
hybrid sat between $0.3$ and $0.9$ across the full $\beta$ range.
The Gaussian SIM baseline failed for a different reason: with
its Gaussian residual it could not match the heavy-tailed real
marginal at any $\beta$, collapsing to $\kappa \approx 1$ and Hill
$\approx 0.22$ regardless of market loading (Fig.~\ref{fig:tails}).
Naive and hybrid clustered together near the real-data kurtosis
reference line ($\kappa_{\real} \approx 13$) on tail metrics, with
the difference between them reflecting the variance inflation under
naive rather than any difference in tail shape. Applying the branch-selection
rule of Algorithm~\ref{alg:hybrid} to the $423$-ticker universe
assigned $421$ tickers to the variance-preserving branch, $2$ to
the $R^2$-preserving branch, and none to the clipped
...
loading ratio $\rho_i = \beta_i^2 \sigma_m^2 / \sigma_{\gen,i}^2$
(Eq.~\eqref{eq:rho}) stayed below $1 - f = 0.90$ for every ticker,
because the JumpHMM marginals carried enough variance that the
market component never crowded out the $f = 0.10$ idiosyncratic
floor. Because the $R^2$-preserving branch had only two empirical
assignments, we validated it on synthetic trackers where the target
$(\beta, R^2)$ was known by construction: across $15$ cells of
$(\beta_{\mathrm{true}}, R^2_{\mathrm{true}}) \in
\{0.8, 1.0, 1.2\} \times \{0.80, 0.85, 0.90, 0.95, 0.99\}$, the
branch selector recovered median $\hat\beta$ and median $\hat R^2$
to within $0.001$ of their targets, and cleared the $\alpha = 0.05$
KS test on at least $99\%$ of replications per cell. To check
...
(Table~\ref{tab:beta_bucket}): the hybrid KS pass rate declined gently from $67\%$ in the
low-$\beta$ quartile to $44\%$ in the high-$\beta$ quartile,
reflecting the caveat noted alongside Algorithm~\ref{alg:hybrid}
that as $\rho_i$ approaches the clipping threshold the
market-driven component of $g$ tightens the marginal toward
Gaussian along its own axis;
...
with the market growth-rate path rescaled by
$\gamma \in \{1, 2, 3\}$, holding the calibrated marginals fixed.
At $\gamma = 1$ no ticker crossed the threshold and the hybrid KS
pass rate matched the unstressed value ($50.3\%$); at $\gamma = 2$
and $\gamma = 3$ the clipping branch activated on $76.4\%$ and
$95.2\%$ of tickers respectively, and the hybrid KS pass rate
rose to $71.8\%$ and $77.3\%$. The clipping branch is therefore
not a degradation under stress but a mechanism that keeps the
composed marginal near the generator target as the market loading
inflates.
