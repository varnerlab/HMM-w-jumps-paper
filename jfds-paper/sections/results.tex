...
where applicable: for each ticker and each replication, a single
innovation path drawn from the ticker's JumpHMM marginal was passed
unchanged to the naive, Gaussian-residual-SIM, and hybrid composers,
so that differences among those three reflected the composition rule
alone, not generator stochasticity. The \emph{Naive} composer plugged
...
idiosyncratic floor. Because the $R^2$-preserving branch had only two empirical
...
pass rate matched the headline value ($50.3\%$); at $\gamma = 2$
