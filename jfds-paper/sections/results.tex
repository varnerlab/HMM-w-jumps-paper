...
in $R^2$ recovery, marginal fit, and tail behavior. Gaussian SIM had
...
variance by $\beta_i^2 \sigma_m^2$, so it too failed nearly all
marginal tests. The hybrid method passed $50.4\%$ of KS replications
...
$66.9\%$ (GARCH(1,1)-$t$) to $75.8\%$ (JumpHMM-on-residuals), higher
than the hybrid's, with correspondingly lower Wasserstein distances.
...
composition without matching the marginal pass rates of methods fitted
directly to residuals, making frozen-parameter evaluation the next
...
method reached an $84.1\%$ mean cross-ticker KS pass rate, against
$78.8\%$ for naive composition, and its median composed-to-observed
variance ratio was $0.97$, whereas naive composition remained
inflated at $1.29$. JumpHMM-on-residuals retained the strongest
marginal fit, with an $86.2\%$ KS pass rate and a slightly lower
median $W_1$ than the hybrid. Matched-length training blocks produced
...
one and kept per-ticker pass rates between $0.3$ and $0.9$ across the
...
$R^2$-preserving assignments were QQQ and SPYG, and both had a $99\%$
...
Taken together, clipping was unnecessary under observed market
variance but stabilized marginal fit when market volatility was
artificially increased, leaving parameter and simulation uncertainty
as the final robustness checks.
...
varied by $0.14$ percentage points across the $25$ grid cells. The
two index trackers remained in the
...
Taken together, the aggregate ordering was insensitive to both the
branch hyperparameters and generator randomness, so the remaining
distinctions among methods concern their modeling and deployment
tradeoffs rather than numerical instability.
