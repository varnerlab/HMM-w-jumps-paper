...
The HMM-WJ model produces a per-ticker marginal validated on SPY
that reproduces heavy tails, regime persistence, and volatility
clustering. Scaling that framework
to a multi-asset universe of synthetic paths requires combining
per-ticker generator output with a common market factor under a
model that recovers each asset's OLS-calibrated coefficients against
SPY and preserves the per-asset marginal. We do this with a
single-index model (SIM) in which SPY's HMM-WJ growth-rate path
serves as the market factor and the per-asset residual is chosen to
...
composes a synthetic per-asset growth-rate path $g_i$ under the SIM:
\begin{equation}
