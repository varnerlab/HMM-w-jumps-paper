...
the construction (values in Table~\ref{tab:hyperparams}); in our
experiments we use $f = 0.10$, reserving at
...
$R^2_{\mathrm{preserve}} = 0.80$ (Table~\ref{tab:hyperparams}). For any asset with $R^2_{i,\real} \ge R^2_{\mathrm{preserve}}$,
...
growth-rate history using the same state-partition and emission
configuration as the SPY analysis (Table~\ref{tab:hyperparams};
risk-free rate $r_f = 0$), holding the jump probability at
$\epsilon = 0$ so each per-ticker generator reduced to the no-jump variant; this
...
Algorithm~\ref{alg:hybrid} at the values in Table~\ref{tab:hyperparams}.
A single innovation path drawn from
