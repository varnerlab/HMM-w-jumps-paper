...
rates $2$--$8$ percentage points lower in-sample
(Table~\ref{tab:model_comparison}). Among the alternatives, each
...
kurtosis fidelity over Normal emissions (Table~\ref{tab:emission_hsmm})
but does not
...
marginal entirely (Table~\ref{tab:aggregate}). The three
...
coverage, differing by less than the cross-ticker spread
(Table~\ref{tab:var}), so hybrid delivers tail accuracy on par with
...
path at the real SPY history, reused the same per-ticker innovation
draw across methods, and skipped the optional cross-sectional rank
reorder, so the comparison isolates the composition rule rather than
validating a full joint market simulator; we also evaluated VaR per
...
(Table~\ref{tab:beta_bucket}), because as the market-driven
...
decomposition attributing the difference to the diversification return
rather than to a calibration
...
work. Composing autoregressive or block-bootstrap residuals with the
single-index factor, instead of independent per-asset draws, would
restore cross-asset residual covariance and address the
diversification-return bias directly. A stress-test harness that
