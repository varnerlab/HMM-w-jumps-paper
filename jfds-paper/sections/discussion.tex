...
distribution), we recommend a uniform location-shift bias correction that
anchors the corrected synthetic median CCGR to a documented long-run prior:
\begin{equation}
...
\end{equation}
where $W_{p,t}$ is path-$p$ wealth at integer step $t$, $\Delta t$ is the
integration step in years, and $\mathrm{CCGR}^{\mathrm{prior}}$ is set from
an external long-run prior (typically near $8\%/\mathrm{yr}$, decomposable
into the prevailing risk-free rate plus an active premium net of
...
single-asset tuning protocol of Section~\ref{sec:method-hmm-tuning},
applied per ticker, would push the hybrid KS pass rates upward at the
