...
rule (Supplementary Table~\ref{tab:cross_asset}). The asset episodes also came from
...
Supplementary Table~\ref{tab:beta_bucket}). Corrected KS pass rates declined from
...
(Supplementary Table~\ref{tab:beta_bucket}). Preserving total variance therefore
...
was doubled or tripled (Supplementary Table~\ref{tab:stress}). Only two tracker
assets used the separate $R^2$-preserving branch
(Supplementary Table~\ref{tab:branch}), so evidence for that branch relied mainly
