...
generation that closes two long-standing gaps in the literature: a
hybrid hidden Markov framework that reproduces single-asset stylized
facts via a Laplace quantile partition and a Poisson jump-duration
override, and a closed-form variance-corrected single-index composer
that lifts those validated marginals into a multi-asset factor
structure without inflating the marginal variance. The construction is
...
and inherit the marginal it was built to deliver. Each composed asset
carries an audit trail (\texttt{hybrid}, \texttt{hybrid-clipped},
\texttt{r2-preserve}) so downstream consumers can inspect the path
selected per ticker without external metadata.
...
Several extensions follow naturally from the present work. The
variance correction generalizes to multi-factor and lagged-factor
single-index models without changing its essential algebra; we leave
