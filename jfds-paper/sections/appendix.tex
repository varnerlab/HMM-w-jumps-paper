...
\caption{\textbf{Diversification-return decomposition on a
    ...
    target weights. The static-to-daily increase of $+27.73$ percentage points on the
...
range assumed for liquid United States equity, shifting the misrepresentation from
...
were defined as follows. The two-sample Kolmogorov--Smirnov (KS) statistic
...
simulated gap of $2.0$ in both direction and size.
...
$0.13$. Supplementary Table~\ref{tab:supp-acf-prediction} compares
...
Supplementary Fig.~\ref{fig:model_internals} shows the fitted hidden Markov
model with jumps (HMM-WJ) for SPY with $N=100$ states. The fitted Laplace
...
(Supplementary Table~\ref{tab:emission_hsmm}): a hidden semi-Markov model
(HSMM) baseline adapted from \citet{bullaBulla2006} ($K = 8$ states,
...
in-sample KS and AD pass rates for the hidden Markov model with no jumps
(HMM-NJ) and HMM-WJ. Pass rates varied
