...
\begin{equation*}
  ...
  = \log\!\left[\sum_i w_i \exp\!\big(g_i(t)\,\Delta t\big)\right].
\end{equation*}
We suppress the time argument, writing $g_i$ for $g_i(t)$. A
second-order Taylor expansion of the exponential about zero gives:
\begin{equation*}
  \exp(g_i\,\Delta t)
  = 1 + g_i\,\Delta t + \tfrac{1}{2}(g_i\,\Delta t)^2
  + O\!\left((g\,\Delta t)^3\right).
\end{equation*}
Substituting this expansion into each summand gives:
\begin{align}
  \sum_{i} w_i \exp\!\big(g_i\,\Delta t\big)
   &= \sum_{i} w_i \!\left[1 + g_i\,\Delta t
        + \tfrac{1}{2}(g_i\,\Delta t)^2\right. \notag \\
   &\hspace{7em}\left.{}+ O\!\big((g\,\Delta t)^3\big)\right] \notag \\
   &= \sum_{i} w_i
...
dispersion into compounded growth. \citet{markowitz1976longRun}
described this as the variance bonus of the long-run mean-variance
allocation; \citet{boothFama1992} called it the diversification return.
...
leaves the variance of those increments unchanged. For path $p$, we
applied the following correction:
\begin{equation}
...
(Table~\ref{tab:emission_hsmm}): the HSMM baseline of
\citet{bullaBulla2006} ($K = 8$ states, negative-binomial
...
\begin{table}[tbp]
  ...
  \label{tab:branch}
  \small
  \resizebox{\textwidth}{!}{\input{sections/tables/table2_by_branch}}
\end{table}
...
\begin{table}[tbp]
  ...
  \small
  \resizebox{\textwidth}{!}{\input{sections/tables/table3_by_beta_bucket}}
\end{table}
...
variance. To derive Eq.~\eqref{eq:scale}, first center the full-return
...
\end{aligned}
\end{equation}
Substituting $\eps_i = s_i\,\tg_i^{\mathrm c}$ gives the two moments:
\begin{align*}
  \Var(\eps_i) &= s_i^2\,\sigma_{\gen,i}^2, \\
  \Cov(g_m,\, \eps_i) &= s_i\,\Cov(g_m,\, \tg_i^{\mathrm c}).
\end{align*}
Under the
...
In Table~\ref{tab:beta_bucket}, the KS pass rate fell from $87.6\%$ in
the lowest-beta quartile to $65.9\%$ in the highest-beta quartile.
