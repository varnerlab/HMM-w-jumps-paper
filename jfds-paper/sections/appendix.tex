...
moment to its generator target. When that target matches the empirical
second moment, the second-order formula for the diversification return
is also matched at the asset level. The remaining difference can
reflect omitted cross-asset residual covariance, higher-order Taylor
terms, calibration drift, and the documented attenuation of realized
diversification returns~\cite{boucheyNemtchinovWong2015}. The
location-shift correction absorbs these effects as a single empirical
drag on log-wealth.
...
The raw median's $\sim\!22$-percentage-point
...
experiment, which we did not run. The gap may also include higher-moment corrections to
...
Std. dev. of $G_t$ (\% yr$^{-1}$) & 214.50 & 220.62 & 214.46 & 205.93 \\
...
(Supplementary Table~\ref{tab:allocator_decomp}). The static-to-daily-rebalance
...
increments. We set $\bar g^{\mathrm{prior}}=8\%/\mathrm{yr}$ before
examining the holdout result. This value was based on external
long-run equity-return estimates and an allowance for fees, turnover,
and market impact~\cite{damodaranERP, ibbotsonSBBI}. We used this
documented prior rather than the realized test year to avoid circular
...
Supplementary Fig.~\ref{fig:model_internals} shows the fitted HMM-WJ components for
...
(Supplementary Table~\ref{tab:emission_hsmm}): an HSMM-style baseline adapted from
\citet{bullaBulla2006} ($K = 8$ states,
...
the distributional distances changed modestly.
...
are reported in Supplementary Table~\ref{tab:cross_asset}. The objective combines
...
In Supplementary Table~\ref{tab:beta_bucket}, the KS pass rate fell from $87.6\%$ in
