...
will probe and removes only a subset of the artifact's consequences
while leaving the structural artifact intact. Inflating per-trade transaction costs
...
Sharpe at daily rebalancing frequency are decision-critical, or where
...
Figure~\ref{fig:model_internals} visualizes the three internal
components of the fitted HMM-WJ on SPY with $N = 100$ states.
Panel~(a) overlays the fitted Laplace cumulative distribution function
on the empirical CDF, with the $99$ vertical lines marking the
equal-probability quantile boundaries $Q_k$ that define the hidden
states; close agreement confirms the Laplace fit is an accurate
marginal model for the SPY excess growth-rate series and validates
the partition rule of Eq.~\eqref{eq:growth_rate}. Panel~(b) displays
...
persistence, with most off-diagonal mass concentrated within $\pm 5$
states of the diagonal. Panel~(c) plots the expected natural
residence time $1/(1 - T_{kk})$ for each state on a $\log_{10}$
scale, with the bottom tail set $\mathcal{S}_{-}$ (states $1$--$5$,
red) and the top tail set $\mathcal{S}_{+}$ (states $96$--$100$,
teal) highlighted. Under pure Markovian dynamics every state, including
...
Table~\ref{tab:emission_hsmm} isolates the contributions of the
emission family and the persistence mechanism by comparing three
variants on SPY: the HSMM baseline of Bulla and Bulla
\cite{bullaBulla2006} ($K = 8$ states, negative-binomial dwell times,
Student-$t$ emissions), HMM-WJ with the original Gaussian emissions
($N = 100$), and HMM-WJ with Student-$t$ emissions ($N = 100$). HMM-WJ
exceeded the HSMM on in-sample distributional fidelity by $15$
percentage points on KS and $49$ percentage points on AD regardless of
emission choice, isolating the contribution of the fine-grained
quantile partition over the coarse $K = 8$ HSMM partition. Holding the
...
The variance-corrected single-index composition of §3.2 states two
closed-form results, the variance scale of Eq.~\eqref{eq:scale} and
the residual-variance target of Eq.~\eqref{eq:r2-target}, and asserts
that the composed series $g_i$ satisfies the three preservation
criteria (SIM recovery, marginal fidelity, cross-sectional
dependence) in the large-$T$ limit. This appendix gives the algebra
behind the two boxed results and verifies the criteria.
