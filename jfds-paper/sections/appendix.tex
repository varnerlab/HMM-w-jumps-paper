...
(Supplementary Fig.~\ref{fig:cov_diag}). Across the $423$ tickers, the full-return
...
$\Delta t\,\mu^2$, against the variance term $D$ of order
$\Delta t\,\sigma^2$; because $\mu^2$ lies more than two orders of magnitude
below $\sigma^2$ for daily growth rates
(Supplementary Table~\ref{tab:descriptive_stats}), we drop it. The realized
annualized compound
...
$\Delta t\,\sigma_m^2$. Across the $423$-ticker universe the calibrated
$\beta$ distribution spans $[0.03, 1.79]$ (Supplementary Fig.~\ref{fig:branch_map}).
The weighted variance of $\beta$ is bounded by one quarter of its
squared range, so $D_{\mathrm{mkt}} \le \tfrac{1}{8}\,\Delta t\,
\sigma_m^2\,(1.79 - 0.03)^2$; at the SPY annualized variance
$\Delta t\,\sigma_m^2 = 0.030~\mathrm{yr}^{-1}$ of the closing-price
series used for the multi-asset universe (Fig.~\ref{fig:preservation}),
$D_{\mathrm{mkt}}$ is at most $0.012~\mathrm{yr}^{-1}$, or $1.2$
...
Supplementary Table~\ref{tab:supp-generator-quantities} collects these values
beside their sample or simulated counterparts.

% --- Table: closed-form quantities at the fitted SPY generator ---
\begin{table}[H]
\centering
\caption{\textbf{Closed-form quantities of the fitted SPY generator
    against sample and simulated values.} Each closed-form entry
    evaluates the expression named in the text at the fitted SPY
    model of Table~\ref{tab:hyperparams}. Sample values are computed
    from the in-sample SPY series; simulated values use $400$
    in-sample-length paths for the jump-episode quantities and
    $1{,}000$ paths for skewness. For comparison with the
    sup-distance entry, the two-sample Kolmogorov--Smirnov critical
    value at $\alpha = 0.05$ for two samples of $T = 2{,}766$
    observations is $1.358\sqrt{2/T} = 0.0365$. Observed
    autocorrelations are those of the in-sample series shown in
    Fig.~\ref{fig:empirical_motivation}d.}
\label{tab:supp-generator-quantities}
\footnotesize
\resizebox{\textwidth}{!}{\input{sections/tables/tableS_generator_quantities}}
\end{table}
...
observations (Supplementary Table~\ref{tab:supp-generator-quantities}). Every within-state mean $\mu_k$ and standard deviation
...
between $0.0058$ and $0.017$ (Supplementary Table~\ref{tab:supp-generator-quantities}). The
...
$0.0631~\mathrm{yr}^{-1}$ (Supplementary Table~\ref{tab:supp-generator-quantities}).
The distributional match is equally direct.
...
(Supplementary Table~\ref{tab:supp-generator-quantities}), where
...
(Supplementary Table~\ref{tab:supp-generator-quantities}),
...
$4.72$ against $4.60~\mathrm{yr}^{-2}$ ($2.17$ against
$2.14~\mathrm{yr}^{-1}$ in standard deviation), and vanishes
under Gaussian emissions ($4.61~\mathrm{yr}^{-2}$).

The fourth moment explains the emission comparison in
Supplementary Table~\ref{tab:emission_hsmm}. Write $G_t = \mu_k +
...
emissions, against an observed value of $7.72$
(Supplementary Table~\ref{tab:supp-generator-quantities}). The emission choice enters
...
Supplementary Table~\ref{tab:emission_hsmm} ($7.5$ and $5.5$) sit
...
(Supplementary Tables~\ref{tab:supp-generator-quantities}
and \ref{tab:descriptive_stats}). Because the states partition the
...
(Supplementary Fig.~\ref{fig:model_internals}c). The autocorrelation
...
matrix (Supplementary Table~\ref{tab:supp-generator-quantities}), $|\theta_2| = 0.253$, and $|\theta_2|^\tau$ falls below
...
(Supplementary Table~\ref{tab:supp-generator-quantities};
Fig.~\ref{fig:empirical_motivation}d). No first-order chain fitted by transition counting can do otherwise:
...
$0.0088$ (Supplementary Table~\ref{tab:supp-generator-quantities}). The product $\epsilon \lambda$ is the quantity that sets
...
gives $24.8\%$ ($99$ of $400$ paths;
Supplementary Table~\ref{tab:supp-generator-quantities}), matching the $\sim\!24\%$
jump-path fraction conditioned on in
Supplementary Fig.~\ref{fig:statistical_validation}. Episodes begin
...
episode; the simulated mean is $3.6\%$
(Supplementary Table~\ref{tab:supp-generator-quantities}), slightly higher through the
...
4.96~\mathrm{yr}^{-1}$ against $\bar a = 1.47~\mathrm{yr}^{-1}$
(Supplementary Table~\ref{tab:supp-generator-quantities}). The
...
$0.13$ (Supplementary Table~\ref{tab:supp-generator-quantities}).
Supplementary Table~\ref{tab:supp-acf-prediction} compares
...
at lag $1$; Supplementary Table~\ref{tab:supp-acf-prediction}) because the free-phase geometric component of
...
\caption{\textbf{Episode-overlap prediction of the jump-path
    ...
    Eq.~\eqref{eq:supp-acf-nj} is excluded from the prediction; it
    has decayed below $0.01$ from lag $5$ onward.}
\label{tab:supp-acf-prediction}
\small
\input{sections/tables/tableS_acf_prediction}
\end{table}
...
with no jumps (Supplementary Table~\ref{tab:supp-generator-quantities}). Forced steps inflate the variance, by about $17\%$ at
...
Supplementary Fig.~\ref{fig:model_internals}.
...
$N=100$ (Supplementary Fig.~\ref{fig:model_internals}). The fitted Laplace CDF and the
...
(Supplementary Table~\ref{tab:emission_hsmm}): a hidden semi-Markov model (HSMM)
baseline adapted from \citet{bullaBulla2006} ($K = 8$ states,
...
from $5.5$ to $7.5$, against an observed value of $7.7$
(Supplementary Table~\ref{tab:descriptive_stats}). The Student-$t$
...
\caption{\textbf{Three-way comparison of emission distributions and
    ...
    is a hidden semi-Markov-style model adapted from \citet{bullaBulla2006}
    ($K = 8$ states, negative-binomial dwell times with a geometric
    fallback, and Student-$t$ emissions); HMM-WJ:
...
(Supplementary Table~\ref{tab:sensitivity_n}). HMM-NJ was insensitive to the
...
were reproducible with these settings (Supplementary Table~\ref{tab:cross_asset}).
...
$65.9\%$ in the highest-beta quartile (Supplementary Table~\ref{tab:beta_bucket}).
...
generator. Centering gives the residual path an exactly zero sample
mean (Eq.~\eqref{eq:center-generator}). On the unclipped non-tracker
branch, using the market path variance in Eq.~\eqref{eq:scale} gives
the residual exactly the remaining variance. The composed variance
also contains the sample cross-covariance term, which need not vanish
on a finite path. Under the independence and large-$T$ assumptions of
Section~\ref{sec:supp-sim-derivations}, the recovered loadings and
intercept converge to their targets. Using the frozen training market
variance, as in the six-method holdout evaluation, adds a further
variance difference if the simulated market variance differs from that
training value. The full-return draw supplies the residual's shape,
but its validation as a standalone asset generator does not establish
that it reproduces the asset's regression-residual distribution.

At $\rho_i = 0$, Eq.~\eqref{eq:scale} gives $s_i = 1$ and the market
term contributes no variance. The composed path then has the same
centered empirical distribution as the generator path, while its mean
is set by the intercept and market term. Equality with the original
uncentered generator distribution would additionally require those
means to agree. For $\rho_i > 0$, the independent market contribution
changes even the centered distribution. To describe that change, take
the unclipped non-tracker branch with population variances and fixed
scales. In population, conditional on the fitted parameters,
...
kurtosis at $\rho_i \in \{0.1, 0.3, 0.5\}$
(Supplementary Table~\ref{tab:supp-kurt-composition}). For a low-loading asset
...
the Gaussian SIM does not (Supplementary Fig.~\ref{fig:tails}), and the hybrid KS
pass rate declines from the lowest to the highest $\beta$ quartile
(Supplementary Table~\ref{tab:beta_bucket}).

The residual-fit methods offer a direct alternative by fitting each
asset's OLS residual series (Table~\ref{tab:aggregate}). Those fitted
series are orthogonal to the observed market by construction, but a
new simulated residual path need not have exactly zero sample
covariance with its market path. Fitting JumpHMM to residuals requires
converting each residual series to a synthetic price path by
cumulative compounding and a second fit per asset. The corrected
full-return construction instead reuses the generator's centered
shape and supplies the residual variance budget without that second
fit. It preserves the composed path mean exactly, while variance and
factor recovery depend on the stated variance convention and
cross-covariance assumptions. At zero market loading it preserves the
generator path's centered empirical distribution. For positive market
loading, Eq.~\eqref{eq:supp-kurt-comp} describes how the independent
market contribution changes population excess kurtosis on the
unclipped non-tracker branch. The clipped and $R^2$-preserving branches
use the different variance allocations in Algorithm~\ref{alg:hybrid}.

% --- Table: simulation check of the kurtosis composition rule ---
\begin{table}[H]
\centering
\caption{\textbf{Simulation check of the kurtosis composition rule.}
    For each variance share $\rho_i$, $200$ pairs of independent
    in-sample-length generator paths were standardized and combined
    as $\sqrt{\rho_i}\, x + \sqrt{1 - \rho_i}\, y$. The predicted
    column evaluates Eq.~\eqref{eq:supp-kurt-comp} with each pair's
    sample excess kurtosis; the simulated column is the sample excess
    kurtosis of the combined series. Both are averaged over the $200$
    pairs.}
\label{tab:supp-kurt-composition}
\small
\input{sections/tables/tableS_kurt_composition}
\end{table}


\section{Uncertainty in the multi-asset jump comparison}
\label{sec:supp-jump-composition}

The jump comparison used four seeds and $1{,}000$ market
replications per window, with all fits frozen (Table~\ref{tab:jump_ablation}). Within each setting,
naive and corrected composition shared the same asset and market
draws. The no-jump and market-only settings reused the no-jump asset
draws; the market-only and market-plus-asset settings reused the
jump-enabled market draws. Enabled and disabled models used separate
random-number streams, so matching replication identifiers did not
imply identical innovations. Primary summaries included every path;
jump-active subsets only described the mechanism.
For each metric, we averaged across tickers within each replication
and formed the difference between settings
(Table~\ref{tab:jump_ablation_uncertainty}). The Monte Carlo standard
error was the sample standard deviation of the $1{,}000$ differences
divided by $\sqrt{1{,}000}$, and intervals used the mean difference
plus or minus $1.96$ standard errors
(Table~\ref{tab:jump_ablation_uncertainty}). Keeping the cross-section
together retained dependence from the shared market draw. The
intervals did not treat assets as independent market histories or
include uncertainty from fitting or historical-window selection.
Kolmogorov--Smirnov (KS) non-rejection remained descriptive because
returns were temporally dependent.
The temporal contrast between training and holdout remained clear
in the replication-level uncertainty estimates
(Table~\ref{tab:jump_ablation_uncertainty}). The episode frequencies
matched the rare-event setting (Supplementary Table~\ref{tab:jump_ablation_episodes}):
$247$ of
$1{,}000$ training-length SPY paths and $22$ of $1{,}000$ holdout-length
paths contained an episode. The corresponding shares across enabled
asset generators were $24.24\%$ and $2.46\%$. Market-only jumps
reduced average $25$-lag autocorrelation error for $99.5\%$ of
training assets but only $2.6\%$ of holdout assets. Enabling jumps in
both components reduced that error for $96.9\%$ and $0.5\%$,
respectively. These fractions described ticker-level mean changes,
not significance tests. The temporal contrast therefore extended
across most assets, within the limits of the short holdout and
transferred SPY calibration.

\begin{table}[H]
  \centering
  \caption{\textbf{Monte Carlo uncertainty in the effect of jump settings
    under corrected composition.} Differences are relative to jumps
    disabled, with $1{,}000$ market replications per window.
    $\Delta$ACF$_{25}$ is the change in mean absolute error of the
    absolute-return autocorrelation function through lag $25$;
    negative values indicate improved temporal fit.
    $\Delta$KS is the change in Kolmogorov--Smirnov non-rejection
    rate in percentage points. Brackets contain approximate $95\%$
    Monte Carlo (MC) intervals conditional on the fitted models and
    observed histories.}
  \label{tab:jump_ablation_uncertainty}
  \footnotesize
  \setlength{\tabcolsep}{4pt}
  \input{sections/tables/tableS_jump_ablation_uncertainty}
\end{table}

\begin{table}[H]
  \centering
  \caption{\textbf{Episode incidence and breadth of the temporal
    change in the multi-asset jump comparison.} SPY paths with an
    episode are counted over the $1{,}000$ jump-enabled market
    replications per window. Asset paths with an episode is the mean
    jump-active share across the enabled asset generators. The last
    two columns give the share of assets whose mean absolute-return
    autocorrelation error through lag $25$ fell under corrected
    composition when jumps were enabled in the market only, or in the
    market and the assets, relative to jumps disabled.}
  \label{tab:jump_ablation_episodes}
  \footnotesize
  \setlength{\tabcolsep}{4pt}
  \resizebox{\textwidth}{!}{\input{sections/tables/tableS_jump_ablation_episodes}}
\end{table}
