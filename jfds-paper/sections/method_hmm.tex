...
%   (with closing bridge sentence to the multi-asset composition).
...
We defined the hybrid hidden Markov model with jump-diffusion
(HMM-WJ) as the tuple
$\mathcal{M} = (\mathcal{S}, \mathcal{O}, \mathbf{T}, \mathbf{E}, \bar{\pi})$
\cite{rabinerJuang1986} (Figure~\ref{fig:model_architecture}), where
the hidden state
...
$\mathcal{O} \subset \mathbb{R}$ consisted of the observed continuous excess
...
matching the concentration of small price movements visible in
the empirical marginal density of
Fig.~\ref{fig:empirical_motivation}(a),
\cite{kotzKozubowskiPodgorski2001, tothJones2019}) and its
...
$\nu$ degrees of freedom, $\mu_k$ is the state-conditional location set
to the sample mean of the in-sample observations assigned to state $k$
under the Laplace-quantile partition, and $\sigma_k$ is a scale
parameter set to the sample standard deviation of those observations.
Because a standard $t_\nu$ has variance $\nu/(\nu-2)$, the emission's
state-conditional standard deviation is $\sigma_k\sqrt{\nu/(\nu-2)} =
\sigma_k\sqrt{5/3}$ rather than $\sigma_k$; $\sigma_k$ is therefore a
scale, not the conditional standard deviation. The degrees-of-freedom
sweep that selects $\nu$ is run under this same parameterization, so
the reported operating point already reflects the induced within-state
variance. The value $\nu = 5$ was
...
overridden to target tail states, with a user-configurable bias
$p_{\rm neg}$ (default $0.52$) that favored negative-tail states to
reproduce gain/loss asymmetry (Figure~\ref{fig:model_architecture}). The empirical
...
is the invariant distribution of $\mathbf{T}$ alone, it approximates
the invariant distribution of the jump-augmented process (whose state
also carries jump status and remaining episode length); the
discrepancy affects only the first few steps of each path and is
negligible at the $2{,}766$-day horizon. At each subsequent
  ...
  \FOR{$t = 1, \dots, T$}
    ...
    \ELSE
      ...
      \ELSE
        ...
        \STATE $S_t \sim \mathrm{Uniform}(\mathcal{J})$;\quad
          $k \gets \min(K, T - t) - 1$
          \hfill \COMMENT{emit first forced step; $K$ steps in total}
      \ENDIF
    \ENDIF
...
while adopting an HSMM-style state-duration parameterization
\cite{bullaBulla2006} in place of the instantaneous-jump formulation
of those classical models. The label \emph{jump-diffusion} here refers
to this heavy-tailed tail-duration override layered on a discrete-state
chain with Student-$t$ emissions; the construction is a discrete-state
Markov generator and carries no continuous-time diffusion component.
Calibration used a multi-objective grid search that minimized the
discrepancy between historical and simulated market signatures. The objective targeted two stylized facts
\cite{cont2001, rydenTerasvirtaAsbrink1998}, both visible in the
...
\cite{cont2001} up to a lag of $L = 25$ trading days (the
short-horizon window in which the empirical ACF of $|G_t|$ carries
most of its variation; post-fit evaluation in Fig.~\ref{fig:model_comparison} extends to lag $252$ for the
visual ACF comparison), and leptokurtosis
...
no-jump generator and carries no information about
$(\epsilon, \lambda)$, so the objective is conditioned on the paths in
which the jump mechanism is active \cite{glasserman2003}. We set the kurtosis-penalty weight to
...
brute-force evaluation. The HMM-WJ framework produced a
