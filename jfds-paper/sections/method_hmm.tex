...
also carries jump status and remaining episode length); this
introduces a small, unquantified transient rather than exact
stationarity of the full generator. At each subsequent
...
the state into a tail set (drawn per step as $\mathcal{S}_{-}$ with
probability $p_{\rm neg}$, else $\mathcal{S}_{+}$) for $K$
...
of those classical models. We describe the mechanism as a
\emph{jump-duration} override rather than jump-diffusion: it is a
heavy-tailed tail-duration override layered on a discrete-state chain
with Student-$t$ emissions, a discrete-state Markov generator that
carries no continuous-time diffusion component.
