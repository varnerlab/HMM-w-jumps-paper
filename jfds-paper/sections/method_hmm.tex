...
movements in Fig.~\ref{fig:empirical_motivation}a~\cite{kotzKozubowskiPodgorski2001, tothJones2019}.
...
$98.3\%$ and $94.8\%$. The full comparison against an HSMM-style
baseline adapted from \citet{bullaBulla2006} is given in
Supplementary Table~\ref{tab:emission_hsmm} of
...
We addressed that problem with occasional jump episodes. Whenever no
jump episode was active, a new episode began with probability
...
$p_{\rm neg}$ and the positive set otherwise. The model made this
choice independently at each forced step, allowing negative extreme
returns to occur slightly more often than positive ones.
Table~\ref{tab:hyperparams} gives all four jump settings.

We obtained the initial-state distribution $\bar{\pi}$ by solving
$\bar{\pi}\mathbf{T}=\bar{\pi}$ with
$\sum_k\bar{\pi}_k=1$~\cite{hamilton2018regime, grinsteadSnell1997}. This
...
values. For each jump-active path, the first part of the objective
measured the squared difference between the observed and simulated
autocorrelation functions of absolute returns through lag $L$. The
second part measured the squared difference in excess kurtosis. We
then averaged both errors over the jump-active paths. The calibration
objective was:
\begin{equation}
    J(\epsilon, \lambda) =
...
for path $r$. We use $\kappa$ for kurtosis to avoid collision with the
jump-duration variable $K$.
