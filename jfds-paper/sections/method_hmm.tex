...
Table~\ref{tab:hyperparams}) as the value that
...
$p_{\rm neg}$ and as $\mathcal{S}_{+}$ otherwise (values in
...
autocorrelation function (ACF) of absolute excess growth rates~\cite{cont2001} up to a lag of $L$ trading days
(value in Table~\ref{tab:hyperparams}), the range where
...
at least one jump occurred~\cite{glasserman2003}. We set the kurtosis-penalty weight $w_\kappa$
to balance tail behavior against the temporal ACF
term and explored $(\epsilon, \lambda)$ by brute-force grid search;
the swept ranges, $w_\kappa$, $L$, and every other HMM-WJ and SIM
composition hyperparameter are collected in
Table~\ref{tab:hyperparams}. The resulting model produced a per-ticker
