...
(Figure~\ref{fig:model_architecture}), where
the hidden state
...
Fig.~\ref{fig:empirical_motivation}(a),~\cite{kotzKozubowskiPodgorski2001, tothJones2019}) and its
...
with Student-$t$ emissions, a Markov generator that
carries no continuous-time diffusion component.
Calibration used a multi-objective grid search that minimized the
discrepancy between historical and simulated market signatures. The objective targeted two stylized facts~\cite{cont2001, rydenTerasvirtaAsbrink1998}, both visible in the
...
autocorrelation function (ACF) of absolute excess growth rates~\cite{cont2001} up to a lag of $L = 25$ trading days (the
