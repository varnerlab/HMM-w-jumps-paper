...
out-of-sample testing. The data included daily open, high, low, and
...
$N_{\rm tail}$ was user-configurable (value in Table~\ref{tab:hyperparams}).
...
but adds no continuous-time diffusion component. We selected
$(\epsilon,\lambda)$ by grid search against two features of the SPY
history in Fig.~\ref{fig:empirical_motivation}: volatility clustering
and kurtosis~\cite{mandelbrot1963, cont2001,
rydenTerasvirtaAsbrink1998}. The first term of the objective measured
the squared error between the observed and simulated ACF of absolute
excess growth rates through lag $L$. The second penalized the
...
paths with at least one jump~\cite{glasserman2003}. The weight
$w_\kappa$ balanced the kurtosis and ACF terms. The search ranges,
