...
%% TikZ architecture diagram moved to supplement for space.

\iffalse
% ── Figure 2: Model Architecture (moved to supplement S10) ──────────────────
\begin{figure}[tp]
\centering
\resizebox{\textwidth}{!}{%
\begin{tikzpicture}[
...
\begin{equation}
    G_{i,t} = \alpha_i + \beta_i \cdot G_{\text{SPY},t} + \eta_{i,t}
    \label{eq:sim}
\end{equation}
where $G_{\text{SPY},t}$ denotes the SPY excess growth rate at time $t$, $\beta_i$ measures the systematic sensitivity of asset $i$ to the index, $\alpha_i$ is the idiosyncratic drift, and $\eta_{i,t}$ is the idiosyncratic shock. The parameters $(\alpha_i, \beta_i)$ were estimated via ordinary least squares on the in-sample time series for each asset $i$, and the empirical residuals $\hat{\eta}_{i,t} = G_{i,t} - \hat{\alpha}_i - \hat{\beta}_i G_{\text{SPY},t}$ were retained for resampling.

Synthetic multi-asset paths were generated in three steps. First, the hybrid HMM produced a single synthetic SPY excess growth rate path $\{\hat{G}_{\text{SPY},t}\}_{t=1}^T$ using the regime-switching and jump-duration mechanism described above. Second, for each asset $i$, an idiosyncratic shock sequence $\{\hat{\eta}_{i,t}\}$ was drawn by resampling the empirical residuals with replacement, preserving their non-Gaussian distribution (empirical residual kurtosis exceeded 10 for several assets). Third, the synthetic excess growth rate for asset $i$ was recovered as:
\begin{equation}
...
The reordering proceeds in three steps. First, we simulate $T$ excess growth rate values independently from each asset's fitted HMM, producing $d$ independent time series. Second, we generate correlated ``rank instructions'' that specify, for each time step and each asset, whether that step should be a high or low value relative to the asset's own distribution. These rank instructions are produced by drawing $T$ samples from a $d$-dimensional distribution (the copula) and converting each sample to a uniform variate via the CDF of that distribution. The choice of copula family determines how the rank instructions behave in the tails: under a Gaussian copula, the draws come from a multivariate Normal, so extreme values across assets rarely coincide; under a Student-t copula, the draws come from a multivariate Student-t distribution with $\nu$ degrees of freedom, whose heavier tails cause extreme values to line up across assets much more frequently. In concrete terms, when the Student-t copula assigns SPY a rank instruction in the 1st percentile (a crash day), it is far more likely to simultaneously assign NVDA, JNJ, and JPM rank instructions in their lower percentiles than the Gaussian copula would, even at the same correlation level. Third, for each asset, we sort its independently simulated HMM values to match the ranks implied by the copula-sampled instructions. The result is a set of $d$ correlated synthetic paths, each with the same marginal distribution as its standalone HMM (because no values are created or removed, only reordered) but with cross-asset co-movement, including tail co-movement, dictated by the copula.
...
The simulation procedure is detailed in Algorithm~\ref{alg:sim} (Online Appendix~S4). At each step, the chain either follows the empirical transition matrix (probability $1-\epsilon$) or enters a Poisson jump episode (probability $\epsilon$) that forces the state into the tail set $\mathcal{S}_{-}$ or $\mathcal{S}_{+}$ for $K \sim \text{Poisson}(\lambda)$ consecutive steps. After the jump, normal Markovian transitions resume. The remaining pipeline stages (model construction, state decoding, and hyperparameter grid search) are also provided in Online Appendix~S4.
