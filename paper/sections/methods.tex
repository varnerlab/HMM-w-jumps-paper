...
The SIM imposed a linear, single-factor dependence structure that could not capture nonlinear or tail dependence between assets. As an alternative, we considered copula-based dependence specifications that separated the marginal distributions from the dependence structure \cite{embrechts_copulas_2002, cherubini_copula_2004}. In the copula framework, an independent HMM was fitted to each asset's price series, producing per-asset marginal models $\mathcal{M}_1, \dots, \mathcal{M}_d$. A copula $C$ was then fitted to the rank-transformed (probability integral transform) growth rates to capture the cross-asset dependence structure (full mathematical details, including copula densities, tail dependence coefficients, vine decomposition, and the simulation algorithm, are given in Supplemental Section~S6). Synthetic paths were generated by sampling from the copula to obtain correlated uniform variates, which were then mapped back to each asset's marginal distribution via rank reordering of independently simulated HMM paths.

We evaluated three copula families. The \textit{Gaussian copula} parameterized dependence through a correlation matrix $\mathbf{\Sigma}$ estimated from the rank-transformed data; it captured linear dependence but imposed zero tail dependence by construction. The \textit{Student-t copula} augmented the correlation matrix with a degrees-of-freedom parameter $\nu$ estimated by profile maximum likelihood, introducing symmetric tail dependence that grew stronger as $\nu$ decreased. The \textit{C-vine copula} \cite{aas_pair_2009} decomposed the $d$-dimensional dependence structure into a hierarchy of bivariate copulas, one per edge of a canonical vine tree. At each edge, five candidate bivariate families (Gaussian, Student-t, Clayton, Gumbel, Frank) were fitted and the family with the lowest Akaike information criterion (AIC) was selected. This construction allowed heterogeneous dependence structures across asset pairs, with Clayton copulas capturing lower-tail dependence, Gumbel copulas capturing upper-tail dependence, and Student-t copulas capturing symmetric tail dependence. The vine ordering was determined by sorting variables by the sum of absolute Kendall's $\tau$ values, placing the most connected variable (typically the market index) at the root.
