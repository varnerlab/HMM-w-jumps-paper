...
Binary pass rates summarize distributional fidelity but discard information about the magnitude of any discrepancy. Two continuous metrics were computed alongside KS and AD to provide effect-size information that is independent of sample size.
...
\end{equation}
$W_1$ is expressed in the same units as the data (daily excess growth rates), equals zero only when the empirical distributions are identical, and is computed by sorting both samples -- the temporal ordering of observations is irrelevant. Smaller values indicate closer distributional agreement.

The Hellinger distance measures the overlap between two probability distributions and is bounded in $[0, 1]$, with $H = 0$ indicating identical distributions and $H = 1$ indicating distributions with completely disjoint support. Given normalized histogram estimates $\{p_k\}_{k=1}^{K}$ and $\{q_k\}_{k=1}^{K}$ of the observed and simulated marginal densities over a common $K$-bin grid:
\begin{equation}
    H(P, Q) = \frac{1}{\sqrt{2}}\sqrt{\sum_{k=1}^{K}\left(\sqrt{p_k} - \sqrt{q_k}\right)^2}
    \label{eq:hellinger}
\end{equation}
The $[0,1]$ scale makes $H$ directly interpretable across models and windows without knowledge of the data's physical units. Histograms were constructed on the common support of observed and simulated samples using $K = 50$ equal-width bins. Both $W_1$ and $H$ were computed path-by-path and averaged across the 1,000 paths; standard errors were estimated as $\mathrm{std}/\sqrt{n_{\mathrm{paths}}}$.
