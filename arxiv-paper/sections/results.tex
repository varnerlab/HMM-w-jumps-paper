...
ordinary state residence times were only $1$--$2$ steps, whereas the
selected jump duration had a mean of $90$ steps
(Fig.~\ref{fig:model_internals}). We then checked whether the results
...
equivalent to HMM-NJ. We then tested whether the distributional fit
...
(Fig.~\ref{fig:jump_mix}). Here $f$ is the share of accepted paths
...
We therefore used $f \approx 0.25$. At this value, simulated kurtosis
stayed within $2\%$ of the observed value, and the autocorrelation
error moved about a quarter of the way from the no-jump value toward
the all-jump endpoint. This choice balanced distributional and
temporal fit in the SPY analysis. The multi-asset experiment that
followed set the jump probability to zero so that it could focus on
how the paths were combined.
...
\begin{figure}[tbp]
  ...
  \caption{\textbf{Jump-mix enrichment frontier for SPY} (in-sample,
    ...
    hidden Markov model with jumps (HMM-WJ) ensemble to a target
    jump-path fraction $f$ (the share of
    accepted paths containing at least one jump) traces the
    temporal-marginal tradeoff. Both panels share the horizontal axis:
    mean absolute error in the autocorrelation function (ACF-MAE) for
    $|G_t|$ over lags
