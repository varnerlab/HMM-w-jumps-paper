...
To explain why daily rebalancing overstated synthetic growth, this
appendix derives the bias correction in
...
To adjust the return level without changing path ranks or increment
variance, we subtracted a constant drift in log-wealth so that the
ensemble median annualized growth rate matched an external prior
...
To determine whether a less intrusive adjustment could remove the
bias, we considered and rejected several alternatives. A
...
override, we inspected the fitted HMM-WJ components for SPY at
...
partition over $N \in \{30, 60, 90, 100, 150, 200, 350\}$ and re-evaluated the
