...
\affil{Cornell University, Ithaca, NY 14850}
\date{\today}
...
The quantitative modeling of financial markets has long faced the challenge of reconciling the theoretical implications of the efficient markets hypothesis (EMH) with the empirical complexity of observed price dynamics. Under the EMH, asset prices fully reflect all available information, implying that price changes follow a martingale process with no exploitable predictability \cite{fama_efficient_1970}. This perspective motivated the widespread adoption of geometric Brownian motion (GBM) as a canonical model for asset prices. However, extensive empirical evidence demonstrates that financial returns systematically deviate from the Gaussian assumptions underlying GBM, exhibiting heavy-tailed behavior, volatility clustering, and intermittent extreme events \cite{mandelbrot_variation_1963, cont_empirical_2001}. These deviations, commonly referred to as stylized facts, are remarkably robust across asset classes, time periods, and market structures.

This study focuses on three such properties: leptokurtic return distributions, the absence of linear autocorrelation in raw returns, and the persistence of volatility. While the lack of linear autocorrelation is broadly consistent with market efficiency, the clustering of volatility reveals long-memory effects in higher-order moments, classically captured by autoregressive conditional heteroskedasticity (ARCH) models \cite{engle_1982}. Understanding why stock market volatility changes over time requires looking beyond simple diffusion into the stochastic nature of price distributions \cite{schwert_why_1989, stein_stock_1991}.

Hidden Markov models (HMMs) and related regime-switching frameworks offer a natural probabilistic structure for capturing non-stationary market behavior through latent state dynamics \cite{rabiner_introduction_1986}. By allowing asset returns to evolve according to unobserved regimes, these models can reproduce non-Gaussian distributions and accommodate changes in market conditions. However, standard HMM specifications often fail to generate sufficient persistence in high-volatility regimes, leading to overly rapid mean reversion following extreme events \cite{ryden_stylized_1998, bulla_stylized_2006}.

The motivation of this work is to develop a generative modeling framework capable of producing synthetic equity return series that faithfully reproduce empirical stylized facts over long horizons. Such models are particularly valuable for stress testing, scenario analysis, and risk management applications, where the goal is the realistic simulation of adverse market conditions \cite{assefa_generating_2021, kwon_can_2024}. We propose a hybrid HMM framework that discretizes continuous excess growth rates into quantile-based regimes and augments the resulting Markov process with a jump-duration mechanism designed to enforce volatility persistence; this bridges the gap between smooth diffusion and the discontinuous shocks identified by machine-learning jump-detection algorithms \cite{au_yeung_jump_2020}.

\section{Materials and Methods}
\label{sec:methods}

\subsection{Dataset and modular pipeline}
The empirical foundation of this study is a comprehensive dataset composed of 424 United States listed equities and exchange-traded funds spanning 2,515 trading days. A modular pipeline was developed to support the automated construction of return models for any ticker within this dataset, allowing for high-throughput simulation and stress testing across diverse market sectors. To validate the performance of this pipeline, we focused our analysis on the representative single asset SPY, which tracks the S\&P 500 index. The data includes daily open, high, low, and close prices along with volume-weighted average price metrics (VWAP). For training purposes, the first 2,515 observations constitute the in-sample dataset, spanning from January 3, 2014, to December 29, 2023. A subsequent 277 trading days, from January 2, 2024, to February 7, 2025, are reserved for out-of-sample backtesting to assess model generalizability. This daily resolution allows the framework to identify the long-term statistical regularities and stylized facts suggested by market microstructure theory.

\subsection{Excess growth rate calculation}

We compute the excess growth rate $R_{i,j}$ for a given equity price series $P_{i,j}$ using VWAP data. In this formulation, the index $i$ represents a specific equity within the dataset, while the index $j$ denotes the discrete time step. In order to isolate the stochastic premium earned over a risk-free investment, the excess growth rate is defined as:
\begin{equation}
...
Calibrating the model with these optimized parameters, $\epsilon \approx 5 \times 10^{-5}$ and $\lambda \approx 67$, allowed the framework to faithfully reproduce the heavy-tailed kurtosis and the ARCH effect observed in the historical SPY dataset. Based on the statistical consistency results, the configuration with $N=90$ states was deemed to be an adequate resolution based on statistical and qualitative consistencies and will be used for synthetic data generation and analysis in this study. Increasing the number of states $N$ enhances the granularity of the market regimes, leading to a more precise fit for in-sample results. While it becomes trivial to add more states, there is a practical limit to this refinement; if $N$ is excessively large, such as 350 states, the empirical estimation protocol fails because the historical dataset becomes too sparse relative to the state space. In such cases, certain states may never be visited, resulting in zero transition probabilities that break the mathematical integrity of the Markov chain. Conversely, a very small number of states, such as $N=10$, results in a coarse-grained model that misses the critical heavy tails observed in the empirical data. Therefore, the selection of $N=90$ provides a sufficient balance between distributional fidelity and statistical robustness. The analysis is divided into in-sample evaluations, spanning from January 3, 2014, to December 29, 2023, totaling 2515 trading days, and out-of-sample backtesting, which covers 277 trading days from January 2, 2024, to February 7, 2025.

\subsection{In-sample performance}

The in-sample analysis evaluates the statistical alignment of simulated returns with historical data across 2515 trading days. We performed Kolmogorov-Smirnov and Anderson-Darling tests for each case using a $p$-value cutoff of 0.05, summarized in Table \ref{tab:statistical_results}.
...
The configuration with $N=90$ states achieves an exceptionally high level of consistency. For the standard model with no jumps, illustrated in Figure \ref{fig:nj_90_combo}, the distribution fit is highly accurate, but the model fails to capture volatility clustering. The autocorrelation of absolute growth in subplot (c) remains flat, proving that state resolution alone cannot overcome the memoryless limitations of standard transition logic.
...
The hybrid model with 90 states, shown in Figure \ref{fig:wj_90_combo}, represents a practical configuration. The simulated absolute growth autocorrelation in subplot (c) mirrors the ARCH effect identified in historical SPY data, aligning with advanced volatility estimation techniques using HMM architectures \cite{rossi_volatility_2006}.
...
To assess generalizability we conducted an out-of-sample backtesting analysis using the hybrid model calibrated with $N=90$ states. This analysis utilized 277 trading days of unseen data from January 2, 2024, to February 7, 2025. The results, presented in Figure \ref{fig:wj_90_oos_combo}, demonstrate that while the model exhibits the anticipated performance degradation compared to the in-sample training data, it remains statistically consistent with real world market behavior. The Kolmogorov-Smirnov test achieved a pass rate of 83.2 percent, confirming the model's ability to capture the median mass of the distribution. The Anderson-Darling test, which is more sensitive to the extreme tail behavior required for risk management, yielded a pass rate of 80.0 percent.

A critical validation metric for the generative integrity of the model is the distribution of distances shown in Figure \ref{fig:oos_correlation_distance}. The correlation distance, defined as $(1-\rho)$, measures the temporal alignment between simulated paths and the observed historical trajectory. The probability density for these results is centered at a mean distance of 1.0. This indicates that the empirical and generated data are linearly unrelated in terms of specific daily timing; this result is favorable in the context of quantitative finance. It confirms that the generative framework produces independent stochastic realizations that capture the stylized facts of the market without violating the efficient markets hypothesis by attempting to predict specific events \cite{fama_efficient_1970}. This underscores the distinction between a generative framework intended for stress testing and a forecasting tool intended for point prediction \cite{assefa_generating_2021, kwon_can_2024}.

Furthermore, we observed a percentage difference in volatility of approximately 31.87 percent between the simulated paths and the observed out-of-sample standard deviation. Despite this gap, the hybrid model successfully preserves the statistical signal of volatility clustering in subplot (c). Unlike the standard Markov transition matrix which decays rapidly, the Poisson jump mechanism successfully mimics the sticky nature of market volatility in unseen conditions. These results validate the hybrid framework as a robust tool for producing synthetic time series that remain statistically consistent with the underlying stochastic process \cite{cont_empirical_2001, ryden_stylized_1998, bulla_stylized_2006}.
...
\WHILE{$counter \le M$}
    ...
    \IF{$u < \epsilon$} 
        ...
        \FOR{$j = 1$ to $K$ while $counter \le M$}
            ...
            \IF{$w < 0.52$} 
                \STATE \COMMENT{Heuristic bias towards negative returns}
                \STATE $S_{counter} \sim \text{Uniform}(\mathcal{S}_{bottom})$ 
            \ELSE
